...
from transformers import AdamW, MBartModel, AutoConfig, MBartConfig, MBart50Tokenizer
...
def load_checkpoint(checkpoint: str, **kwargs):
    ...
    if os.path.exists(hparam_yaml_file):
        ...
        model = MBartModelQE.load_from_checkpoint(checkpoint, hparams={**hparams, **kwargs})
    else:
...
class Tokenizer:
    ...
    def __init__(self, pretrained_model, src_lang=None, tgt_lang=None):
        self.tokenizer = MBart50Tokenizer.from_pretrained(pretrained_model, src_lang=src_lang, tgt_lang=tgt_lang)
        ...
        self.configs = MBartConfig.from_pretrained(pretrained_model)
    ...
    def batch_encode(self, sources: list, hypothesis: list):
        ...
        for mt, src in zip(hypothesis, sources):
            ...
            if len(src_attention_mask) > self.max_positions:
                src_input_ids = src_input_ids[:self.max_positions]
                src_attention_mask = src_attention_mask[:self.max_positions]
            ...
            with self.tokenizer.as_target_tokenizer():
                ...
                if len(mt_attention_mask) > self.max_positions:
                    mt_input_ids = mt_input_ids[:self.max_positions]
                    mt_attention_mask = mt_attention_mask[:self.max_positions]
...
class MBartModelQE(pl.LightningModule):

    class ModelConfig(Config):
        pretrained_model: str = "facebook/mbart-large-50"
    ...
    def __init__(self, hparams: Namespace):
        ...
        # mbart
        self.src_lang = self.hparams.get('src_lang', None)
        self.tgt_lang = self.hparams.get('tgt_lang', None)
    ...
    def forward(self,
        ...
        if return_hidden_states:
            if return_attentions:
                attns = (output.encoder_attentions, output.cross_attentions, output.decoder_attentions)
                hds = (output.encoder_hidden_states, output.decoder_hidden_states, hidden_states)
                return mt_score, attns, hds
