...
from transformers import AdamW, RemBertTokenizer, RemBertModel, RemBertConfig
...
def load_checkpoint(checkpoint: str, **kwargs):
    ...
    if os.path.exists(hparam_yaml_file):
        ...
        model = RemBERTModelQE.load_from_checkpoint(checkpoint, hparams={**hparams, **kwargs})
    else:
...
class Tokenizer:
    ...
    def __init__(self, pretrained_model):
        self.tokenizer = RemBertTokenizer.from_pretrained(pretrained_model)
        ...
        self.configs = RemBertConfig.from_pretrained(pretrained_model)
    ...
    def batch_encode(self, sources: list, hypothesis: list):
        ...
        for mt, src in zip(hypothesis, sources):
            inputs = self.tokenizer(src, mt)
            input_ids = inputs['input_ids']
            token_type_ids = inputs['token_type_ids']
            attention_mask = inputs['attention_mask']
            if len(attention_mask) > self.max_positions-2:
                input_ids = input_ids[:self.max_positions-2]
                token_type_ids = token_type_ids[:self.max_positions-2]
                attention_mask = attention_mask[:self.max_positions-2]
            fp_mask = [int(s.startswith("▁")) for s in self.tokenizer.convert_ids_to_tokens(input_ids)]
            encoded_batch["input_ids"].append(input_ids)
            encoded_batch["attention_mask"].append(attention_mask)
            encoded_batch["token_type_ids"].append(token_type_ids)
            encoded_batch["first_piece_mask"].append(fp_mask)
        model_input = {}
...
class RemBERTModelQE(pl.LightningModule):

    class ModelConfig(Config):
        pretrained_model: str = "google/rembert"
    ...
    def __init__(self, hparams: Namespace):
        ...
        self.alpha_merge = self.hparams.get('alpha_merge', 0.5)
        ...
        self.model = RemBertModel.from_pretrained(
    ...
    def forward(
        ...
        if self.selected_layers == 'mix':
            hidden_states = self.scalar_mix(output.hidden_states, attention_mask)
        else:
            selected_layers = list(map(int, self.selected_layers.split(',')))
            hidden_states = torch.stack(output.hidden_states)[selected_layers].mean(dim=0)

        mt_summary = masked_average(hidden_states, token_type_ids.bool() & attention_mask.bool())
        src_summary = masked_average(hidden_states, (~token_type_ids.bool()) & attention_mask.bool())
        ...
        if return_hidden_states:
            if return_attentions:
                return mt_score, wl_scores, output.attentions, output.hidden_states + (hidden_states,)
            return mt_score, wl_scores, output.hidden_states + (hidden_states,)
        if return_attentions:
            return mt_score, wl_scores, output.attentions
    ...
    def training_step(
        ...
        sl_loss_value = self.loss_fn(predicted_scores, batch_target["score"])
    ...
    def prepare_sample(
        ...
        word_tags = [torch.tensor(s['src_tags'] + s['tgt_tags']) for s in sample]
        fs_mask_word_tags = [torch.tensor([1]*len(s['src_tags']) + [2]*len(s['tgt_tags'])) for s in sample]
