...
def get_valid_explanations(explanations, hs_mask, fs_mask, fp_mask):
...
if __name__ == "__main__":
    ...
    # 1) Load Checkpoint
    model = load_checkpoint(args.checkpoint, return_hidden_states=True)
    model.eval()
    model.zero_grad()
    model.to(device)
    ...
    num_layers = model.model.config.num_hidden_layers + 2  # + emb layer + scalar mix layer
    ...
    for i, batched_sample in enumerate(batches):
        ...
        # predict
        with torch.no_grad():
            pred_score, hs = model.forward(**batch, return_attentions=True)
            hs = torch.stack(hs)
        ...
        # (num_layers, bs, seq_len, hdim) -> (bs, num_layers, seq_len, hdim)
        hs = hs.transpose(0, 1)
        bs, seq_len = hs_mask.shape

        for layer_id in range(num_layers):
            # layer
            hs_norm = hs[:, layer_id].norm(p=2, dim=-1)
            explanations = get_valid_explanations(hs_norm, hs_mask, fs_mask, fp_mask)
        ...
        hs_norm = hs.norm(p=2, dim=-1)
        hs_layers = (hs_norm[:, 1:-1] * layer_weights.view(1, -1, 1)).sum(1)
        explanations = get_valid_explanations(hs_layers, hs_mask, fs_mask, fp_mask)
